...
# Configure whether notifications could be critical or not
[[ -z "$AUTO_NOTIFY_CRITICAL_ENABLED" ]] &&
    export AUTO_NOTIFY_CRITICAL_ENABLED=1
...
function _auto_notify_message() {
    ...
    if [[ "$platform" == "Linux" ]]; then
        ...
        if [[ "$exit_code" -eq 130 ]]; then
            if [[ "$AUTO_NOTIFY_CRITICAL_ENABLED" -eq 1 ]]; then
                urgency="critical"
            fi
